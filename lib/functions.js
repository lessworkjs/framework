      ...
      _method.forEach((method) => {
        ...
        if (args) {
          ...
          if ((args.length === 3 && typeof args[args.length - 1] === 'object') || args.length === 4) {
            this.config = args[args.length - 1];
          }
        ...
        if (serverless.plugins.includes('serverless-aws-documentation')) {
...
module.exports = () => {
  ...
  const dirs = require(path.resolve(process.cwd(), 'config/app')).functions;
